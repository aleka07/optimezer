...
# --- НОВЫЕ определения для индивидуальных машин ---
NUM_COMBINERS = 2
NUM_MIXERS = 3
NUM_FORMERS = 2

all_combiners = [f'combiner_{i}' for i in range(NUM_COMBINERS)]
all_mixers = [f'mixer_{i}' for i in range(NUM_MIXERS)]
all_formers = [f'former_{i}' for i in range(NUM_FORMERS)]

machines_available_cumulative = {
    "Расстойка": 8,
    "Выпекание": 6,
    "Остывание": 25,
}
...
MAX_WAIT_FORMING_PROOFING_MIN = 120
MAX_WAIT_PROOFING_BAKING_MIN = 120
...
OUTPUT_CSV_FILE = 'production_schedule_v3_individual_machines.csv'
OUTPUT_TXT_FILE = 'production_summary_v3_individual_machines.txt'
...
for i, batch_info in enumerate(all_batches):
    ...
    product_name = batch_info['product']
    for task_info in batch_info['tasks']:
        ...
        task_vars[i][stage_idx] = {
            "start": start_var, "end": end_var, "interval": interval_var,
            "product": product_name, "stage_name": stage_name, "batch_id": batch_id
        }
        task_lookup[(batch_id, stage_name)] = task_vars[i][stage_idx]
        # Индивидуальные машины для первых 3 этапов
        if stage_name == "Комбинирование":
            machine_specific_intervals = []
            machine_assignment_vars = []
            for combiner_idx in range(NUM_COMBINERS):
                machine_suffix = f'{suffix}_on_combiner_{combiner_idx}'
                lit_assigned = model.NewBoolVar(f'assigned{machine_suffix}')
                opt_interval = model.NewOptionalIntervalVar(
                    start_var, duration, end_var, lit_assigned, f'opt_interval{machine_suffix}'
                )
                machine_specific_intervals.append(opt_interval)
                machine_assignment_vars.append(lit_assigned)
                machine_intervals[all_combiners[combiner_idx]].append(opt_interval)
            model.AddExactlyOne(machine_assignment_vars)
            task_vars[i][stage_idx]['machine_assignment_vars'] = machine_assignment_vars
            task_vars[i][stage_idx]['machine_specific_intervals'] = machine_specific_intervals
        elif stage_name == "Смешивание":
            machine_specific_intervals = []
            machine_assignment_vars = []
            for mixer_idx in range(NUM_MIXERS):
                machine_suffix = f'{suffix}_on_mixer_{mixer_idx}'
                lit_assigned = model.NewBoolVar(f'assigned{machine_suffix}')
                opt_interval = model.NewOptionalIntervalVar(
                    start_var, duration, end_var, lit_assigned, f'opt_interval{machine_suffix}'
                )
                machine_specific_intervals.append(opt_interval)
                machine_assignment_vars.append(lit_assigned)
                machine_intervals[all_mixers[mixer_idx]].append(opt_interval)
            model.AddExactlyOne(machine_assignment_vars)
            task_vars[i][stage_idx]['machine_assignment_vars'] = machine_assignment_vars
            task_vars[i][stage_idx]['machine_specific_intervals'] = machine_specific_intervals
        elif stage_name == "Формовка":
            machine_specific_intervals = []
            machine_assignment_vars = []
            for former_idx in range(NUM_FORMERS):
                machine_suffix = f'{suffix}_on_former_{former_idx}'
                lit_assigned = model.NewBoolVar(f'assigned{machine_suffix}')
                opt_interval = model.NewOptionalIntervalVar(
                    start_var, duration, end_var, lit_assigned, f'opt_interval{machine_suffix}'
                )
                machine_specific_intervals.append(opt_interval)
                machine_assignment_vars.append(lit_assigned)
                machine_intervals[all_formers[former_idx]].append(opt_interval)
            model.AddExactlyOne(machine_assignment_vars)
            task_vars[i][stage_idx]['machine_assignment_vars'] = machine_assignment_vars
            task_vars[i][stage_idx]['machine_specific_intervals'] = machine_specific_intervals
...
# a) Sequence
for i, batch_info in enumerate(all_batches):
    sorted_tasks_for_batch = sorted(batch_info['tasks'], key=lambda t: t['stage_index'])
    for k in range(len(sorted_tasks_for_batch) - 1):
        curr_task_def = sorted_tasks_for_batch[k]
        next_task_def = sorted_tasks_for_batch[k+1]
        curr_idx = curr_task_def['stage_index']
        next_idx = next_task_def['stage_index']
        if curr_idx in task_vars[i] and next_idx in task_vars[i]:
            model.Add(task_vars[i][next_idx]['start'] >= task_vars[i][curr_idx]['end'])

# b.1) Индивидуальные машины для первых 3 этапов
for machine_name in all_combiners + all_mixers + all_formers:
    if machine_intervals[machine_name]:
        model.AddNoOverlap(machine_intervals[machine_name])

# b.2) Кумулятивные ресурсы для последующих этапов
for stage_index, stage_name in enumerate(STAGES):
    if stage_name in machines_available_cumulative:
        machine_count = machines_available_cumulative.get(stage_name)
        if machine_count is None or machine_count <= 0:
            continue
        intervals_for_stage = []
        for i, batch_info in enumerate(all_batches):
            if stage_index in task_vars[i] and task_vars[i][stage_index]['stage_name'] == stage_name:
                intervals_for_stage.append(task_vars[i][stage_index]['interval'])
...
for i, batch_info in enumerate(all_batches):
    ...
    for stage_before, stage_after, max_wait in critical_constraints_defined:
        ...
        if task_before_key in task_lookup and task_after_key in task_lookup:
            model.Add(task_lookup[task_after_key]['start'] - task_lookup[task_before_key]['end'] <= max_wait)
...
for i, batch_info in enumerate(all_batches):
    if batch_info['tasks']:
        actual_last_stage_idx = batch_info['tasks'][-1]['stage_index']
        if actual_last_stage_idx in task_vars[i]:
            last_stage_tasks_ends.append(task_vars[i][actual_last_stage_idx]['end'])
...
if status == cp_model.OPTIMAL or status == cp_model.FEASIBLE:
    ...
    for i, batch in enumerate(all_batches):
        ...
        for task_info in batch['tasks']:
            ...
            assigned_machine = ''
            if stage_name == "Комбинирование":
                for comb_idx, lit_var in enumerate(task_vars[i][stage_idx]['machine_assignment_vars']):
                    if solver.Value(lit_var):
                        assigned_machine = all_combiners[comb_idx]
                        break
            elif stage_name == "Смешивание":
                for mix_idx, lit_var in enumerate(task_vars[i][stage_idx]['machine_assignment_vars']):
                    if solver.Value(lit_var):
                        assigned_machine = all_mixers[mix_idx]
                        break
            elif stage_name == "Формовка":
                for form_idx, lit_var in enumerate(task_vars[i][stage_idx]['machine_assignment_vars']):
                    if solver.Value(lit_var):
                        assigned_machine = all_formers[form_idx]
                        break
            if stage_idx in task_vars[i]:
                start_val = solver.Value(task_vars[i][stage_idx]['start'])
                end_val = solver.Value(task_vars[i][stage_idx]['end'])
    ...
    csv_fieldnames = ["Batch_ID", "Stage", "Start_Time_Min", "End_Time_Min", "Duration_Min", "Assigned_Machine"]
    try:
    ...
    try:
        with open(OUTPUT_TXT_FILE, 'w', encoding='utf-8') as txtfile:
            ...
            for stage_name_res, count in machines_available_cumulative.items():
                txtfile.write(f"  - {stage_name_res}: {count}\n")
            txtfile.write(f"\nИндивидуальные машины:\n")
            txtfile.write(f"  - Комбинирование: {', '.join(all_combiners)}\n")
            txtfile.write(f"  - Смешивание: {', '.join(all_mixers)}\n")
            txtfile.write(f"  - Формовка: {', '.join(all_formers)}\n")

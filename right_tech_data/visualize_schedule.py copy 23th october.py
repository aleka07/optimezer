...
# OUTPUT_IMAGE_FILE = 'gantt_chart_copy_last.png'
...
OUTPUT_IMAGE_FILE = 'fifo_summary_correct_copy_last.png'
...
def create_gantt_chart(csv_path, output_path):
    ...
    fig, ax = plt.subplots(figsize=(40, max(20, len(unique_batches) * 0.9)))
    ...
    for index, row in df.iterrows():
        ...
        ax.barh(
            ...
            left=row['start_min'],
            height=0.9,  # Увеличиваем высоту баров
            color=stage_colors.get(stage, 'grey'),
            edgecolor='black',
            linewidth=1.5  # Увеличиваем толщину границ для лучшей видимости
        )

    # --- 4. Настройка внешнего вида диаграммы ---
    # Настройка оси Y (названия партий)
    ax.set_yticks(range(len(unique_batches)))
    ax.set_yticklabels(unique_batches, fontsize=20, fontweight='bold')  # Увеличиваем размер шрифта
    ax.invert_yaxis()  # Первая партия будет сверху

    # Настройка оси X (время в минутах)
    ax.set_xlabel('Время (минуты)', fontsize=24, fontweight='bold')
    ...
    ax.set_title(title_str, fontsize=28, fontweight='bold', pad=40)
    ...
    ax.tick_params(axis='x', which='minor', labelsize=18, width=2, length=7)
    
    # УБИРАЕМ легенду справа - её не будет на графике

    # Оптимизация расположения элементов - используем всё пространство
